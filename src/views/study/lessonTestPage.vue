...
async function submit() {
  ...
  if (answer.includes('') || answer.length != answerItem.length) {
        ...
        ifZYAnswer(answerItem[i - 1].id.toString(), taoid, userid, answer[i - 1], '').then((res) => {
          ...
          if (i == 1) {
            setTimeout(() => {
              error.sort()
              error.push(taoid)
              router.replace('/study/testReport/' + error.toString() + '/zykc')
        ...
        ifKCAnswer(answerItem[i - 1].id.toString(), taoid, userid, answer[i - 1], '').then((res) => {
          ...
          if (i == 1) {
            setTimeout(() => {
              error.sort()
              error.push(taoid)
              router.replace('/study/testReport/' + error.toString() + '/kc')
              error.splice(0, error.length);
            }, 500)
          }
        })
