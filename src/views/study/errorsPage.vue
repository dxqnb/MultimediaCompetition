...
import {ctkczj, ctzyzj, getMyKcWorng, getMyZyWorng, getTestKcTiList, getTestZyTiList} from "@/api/study";
...
async function smartAnalysis(id: number) {
  const loading = await loadingController.create({
    message: '小U正在努力分析...',
    // duration: 3000,
  });
  await loading.present();
  question.value = errorList[id].problem
  answer.value = errorList[id].answer

  A.value = errorList[id].optionA
  B.value = errorList[id].optionB
  C.value = errorList[id].optionC
  D.value = errorList[id].optionD
  if (type.value == 'zykc') {
    ctzyzj(userid, errorList[id].taoid, errorList[id].tiid).then(async (res) => {
      smart.value = marked(res.data.result)
      await loading.dismiss()
      popoverFlag.value = true
    })
  } else {
    ctkczj(userid, errorList[id].taoid, errorList[id].tiid).then(async (res) => {
      smart.value = marked(res.data.result)
      await loading.dismiss()
      popoverFlag.value = true
    })
...
<template>
  <IonPage>
    ...
    <ion-content :fullscreen="true" class="ion-padding">
      <div style="width: 100%;">
        <ion-segment style="width: 200px;min-width: 0;margin: 0 auto;" @ionChange="change()" v-model="type">
          <ion-segment-button style="min-width: 100px;min-height: 20px" value="zykc">
            <ion-label style="font-size: 13px">专业课程</ion-label>
          </ion-segment-button>
          <ion-segment-button style="min-width: 100px;min-height: 20px" value="kc">
            <ion-label style="font-size: 13px">普通课程</ion-label>
          </ion-segment-button>
        </ion-segment>
      </div>
      <div style="width: 96%;margin: 10px auto;border-radius: 10px;display: flex;position:relative;"
        ...
        <ion-button
            style="--background: #6776FF;--color: white;height: 26px;min-height: auto;font-size: 14px;margin-top: 12px"
            @click="smartAnalysis(i)">智能分析
        </ion-button>
      </div>
    ...
    <div ref="popover">
      <ion-popover style="--width: 312px" @ionPopoverDidDismiss="popoverFlag=false" :is-open="popoverFlag"
        ...
        <div style="background: #FFFFFF;overflow: hidden;width: 100%;position: relative">
          ...
          <ion-content class="vice">
            <ion-text
                style="font-size: 13px;font-weight: 900;color: #808080;text-align: center;width: 100%;display: block;padding: 0 26px">
              {{ question }}
            </ion-text>
            <ion-text
                style="font-size: 13px;font-weight: 900;text-align: center;width: 100%;display: block;padding: 0 26px;"
                :style="answer=='A'?'color: #20bd20':'color: #808080;'">
              A:{{ A }}
            </ion-text>
            <ion-text
                style="font-size: 13px;font-weight: 900;text-align: center;width: 100%;display: block;padding: 0 26px"
                :style="answer=='B'?'color: #20bd20':'color: #808080;'">
              B:{{ B }}
            </ion-text>
            <ion-text
                style="font-size: 13px;font-weight: 900;text-align: center;width: 100%;display: block;padding: 0 26px"
                :style="answer=='C'?'color: #20bd20':'color: #808080;'">
              C:{{ C }}
            </ion-text>
            <ion-text
                style="font-size: 13px;font-weight: 900;text-align: center;width: 100%;display: block;padding: 0 26px 20px 26px"
                :style="answer=='D'?'color: #20bd20':'color: #808080;'">
              D:{{ D }}
            </ion-text>
            <ion-text v-html="smart"
                style="font-size: 13px;font-weight: 900;color: #808080;width: 100%;display: block;padding: 0 26px 40px 26px">
            </ion-text>
          </ion-content>
        </div>
